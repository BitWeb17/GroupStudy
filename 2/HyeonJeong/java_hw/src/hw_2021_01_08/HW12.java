...
class WorkRate {
    ...
    public WorkRate(float hourA, float productionA,
        ...
        amount_of_work = (rateA + rateB) * complete_hour;
    }

    public void calcRemainHour(int accident_day) {
        fireStop(rateA, accident_day);

        total_hour = (amount_of_work / (rateA + rateB)) +
                accident_day * DAY_HOUR;
    }

    public void fireStop(float rate, int day) {
        amount_of_work -= rateB * day * DAY_HOUR;
    }
