...
public class CreateTeam {
    ...
    private String[] AteamArr; // String 형 AteamArr 배열 생성
    private String[] BteamArr;

    private int AnumOfPerson; // AnumOfPerson, BnumOfPerson, numOfTeam 변수 선언
    private int numOfTeam;
    private int BnumOfPerson;

    public CreateTeam(String[] arrA, String[] arrB, final int TEAMNUMBER) {
        // 생성자 CreateTeam, 매개변수 선언 arrA배열(nameA), arrB배열(nameB), TEAMNUMBER
        AteamArr = arrA; // AteamArr 변수에 A팀 배열에 대입
        BteamArr = arrB; // B팀 배열에 대입

        AnumOfPerson = arrA.length; // AnumOfPerson 변수에 arrA.length 길이 만큼 대입
        BnumOfPerson = arrB.length;

        numOfTeam = TEAMNUMBER; // final int TEAMNUMBER 입력받은 값을 numOfTeam에 대입

        AteamArrayList = new ArrayList<String>(); // String 형 AteamArrayList 의 ArrayList 배열 생성??
        BteamArrayList = new ArrayList<String>();
    }

    public void allocRandomTeam() { // 랜덤으로 팀을 할당하는 매서드
