...
public class ThirdThreadTest implements Runnable {
    ...
    public static void main(String[] args) {
        ...
        // 데몬 프로세스(악마에게 영혼을팔아 절대 죽지 않는 프로세스) : init(1) 프로세스가 직접 개입하여 심판을 내리기 전까지는 죽지 않은 불멸의 프로세스
        //                                              (그래서 서비스 운영에서 밥먹듯이 사용됨 -서비스가 다운 되지 않도록 만들기 위해)
        t2.setDaemon(true);

        t1.start();

        // main()를 sleep() 하고자 한다면
        // 반드시 Thread의 동작을 모두 완료한 이후에 sleep() 시키도록 한다
        try{
            Thread.sleep(800); // 메인클래스에 놓은 쓰레드 이거를 여기다 두면 메인이 0.8초동안 t2의 값을 안 받음.
                                     // 그렇기 떄문에 중간에 놓으면 안됨
