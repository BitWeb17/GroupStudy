...
//    }
    ...
    public void allocRandomTeam(){
             // 문제 풀때 팁 )
        // 1. 우선 이문데를 풀기 위한 전략을 세워야한다.
        // 2. 문제의 요구 사항을 파악한다
        // 3. 요구 사항을 하나 하나 격파한다

        // 위의 방식을 따라서 현재 문제에 대한 요구 사항을 파악해보자
        // 문제 : 우리반 사람들을 4개의 팀으로 랜덤하게 할당한다
        // 1) 갠덤을 사용해야 한다(Math.random()) 혹은 랜덤 클래스 사용
        // 2) 우리반 사람이 몇명인가? 17명
        // 2-2) 현재 교차 대면 수업을 하므로 a조,b조가 구분되어 있는 상태다
        //      A조는 A조 내에서 분할 B는 B조 내에서 분할
        //      입력 배열이 1개가 아니고 2개의 입력 배열이 필요하다
        // 2-3) 현재 케이스는 굉장히 특수한 케이스이므로 일반적인 상황에서 배열 1개면 충분 할듯
        // 3) 몇개의 팀으로 구성해야 하는가? 4개의 팀
        // 4) 만약 상황이 된다면 n 개의 팀도 가능할까? 여기까지 고려 하면 좋음!!!!!
        // 5) 그렇다면 17명을 어떻게 4개의 팀으로 나눌까?
        //    17 나누기 4를 하고 랜덤한 특정 번호에 5명을 배치하게 해볼까?
        // 6) 랜덤한 팀원수를 가지는 팀을 분할 했다면 이제 실제 random 값을 추출하여 ArrayList에 정보를 배치한다.
        // 7) 중복이 되면 안되므로 중복을 판정하는 루틴이 추가적으로 필요하다;

        // 프로젝트 ; ~~을 만들어서 돈을 법시다
        // ~을 어떻게 만들까?
        // 앞으로 숙제 할때 요구 사항을 분석하는 일을 해보자!
             // 1. 배열을 백업 받았으므로 랜덤값을 뽑아서 해당 값을 인덱스로 특정 인물을 추출하여 ArrayList에 설정한다.
/*
