...
public class SocketManager {
    ...
    public void send(Socket[] sock, int num) throws IOException {
        ...
        // sendCnt는 송신한 클라이언트 수를 세기 위한 변수
    ...
    // 서버에서 송신한 가위바위보 게임 결과를
    // 클라이언트 측에서 수신하여 화면에 출력하는 메서드
    public void recv(Socket sock) throws IOException {
        // 서버에서 송신한 값을
        // 클라이언트 측에서 바이트 단위로 받아오기 위한 배열 in
        ...
        // String 형으로 받아온 값(가위바위보 게임 결과)을
        // 클라이언트 화면에 출력
        System.out.println(reader.readLine());
    }

    // 클라이언트 소켓의 연결을 끊는 메서드
    public void close(Socket sock) throws IOException {
        sock.close();
    }

    // OR 연산을 통해 무승부인 경우를 판단하는 메서드
