...
public class Ticketing {
    ...
    public Ticketing(int numOfHuman, int numOfTicket) {
        ...
        // 약간 손 볼 곳이 많으므로
        // 그냥 둘 다 할당하도록 한다.
        personNumberArr = new int[numOfHuman];
        ticketNumberArr = new int[numOfTicket];

        personArrayList = new ArrayList<Integer>(numOfHuman);
        ticketArrayList = new ArrayList<Integer>(numOfTicket);
    }
    ...
    public void allocArrayListTicket(int personNum) {
        boolean isDup = false;
        int randNum;
        ...
        do {
            ...
            if (ticketArrayList.contains(randNum)) {
                isDup = true;
            } else {
                isDup = false;
                ticketArrayList.add(randNum, personNum);
            }
        } while (isDup);
    }
