...
public class RandomAlloc {
    ...
    public void setRandomNumber(final int ORDER) {
        ...
        // 루프를 20번만 돌게 되므로
        // 중복 시 continue 를 하게 되면 반복 횟수가 차감되므로
        // 모든 값을 할당하지 못 함 !
        // 작업이 종료되었음을 알려주는 변수가 필요하다.
        boolean finish = false;
        // 중복되지 않는 케이스에서는 20개의 할당이면 끝임을 알려야 한다.
        int cnt = 0;
        // for (int i = 0; i < employee.length; i++) {
    ...
    public void printPrice() {
        ...
        for (int i = 0; i < employee.length; i++) {
            if (employee[i] % 2 == 0) {
                System.out.println("내년을 기약하세요 ㅠㅠ 꽝 입니다.");
            } else if (employee[i] % 7 == 0) {
                System.out.println("당첨 입니다 ~ 통장으로 1000만 원이 입금되었습니다.");
            } else if (employee[i] % 3 == 0) {
                System.out.println("당첨 입니다 ~ 통장으로 100만 원이 입금되었습니다.");
            }
        }
    }
}
