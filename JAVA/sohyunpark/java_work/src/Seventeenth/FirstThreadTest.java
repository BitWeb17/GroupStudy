...
class Worker implements Runnable {
    ...
    public void run() {
        ...
        try {
            ...
            // A1 : 스레드도 일종의 프로세스니까 제어권 위임시 끼어들 수 있음
            // A : 기본적으로 시분할 스케줄링에서 아직 일처리할 것들이 많은데
            //     시간이 만료되어 제어권이 다른 녀석에게 양도되었다.
            //     그러면 제어권을 양도한 녀석은 어디에 있을까?
            //     Wait Queue라는 대기열에서 자신의 차례가 올 때까지 대기하고 있다.
            //     스케줄링할 때는 이 Wait Queue에서 스케줄링을 하게 된다.
            //     스케줄링에서 현제 프로세스를 제외합니다란 뜻을
            //     내부적으로 내포하고 있다.
...
public class FirstThreadTest {
    public static void main(String[] args) {
        ...
        // 수업 중에 프로세스는 무엇의 추성화라고 했었는지 기억이 나는가?
        // CPU의 추상화라고 하였다.
        // 왜? CPU가 기계어를 구동시켜야지만 프로그램 코드가 돌아가니까
        // (운영체제 개론을 한 번 공부해보는 것을 권장함)

        // | 음악 듣기 | 게임하기 | 프로그래밍하기 |
        // 동시에 실행하는 것처럼 느끼게 만듬 (하나씩 시분할 방식(프로세스 실행 방식)으로 동작됨)
        // 프로세스 : 현 시점에 CPU가 구동시키고 있는 코드
        // 인터페이스 : 메소드를 추상화시키는 것
        ...
        // CPU가 여러 프로세스들을 동시에 돌아가는 것처럼 보이게 만들기 위해
        // 시분할 방식으로 우선순위를 줘서 돌립니다 : 구체
        // 여러 프로세스들이 실행됩니다 : 추상화

        // CPU가 프로그램을 실행합니다 : 구체
        // 프로세스 : 추상화

        // Q : 인터페이스가 추상화를 잘해야한다는 것이
        //     인터페이스를 대충 훑어봐도 이게 뭘 하는구나란 생각이 들어야 하는건가요?
        // A : 추상화와 가독성은 다소 차이가 있다.
        //     추상화가 잘 되었다는 것은 이런 의미다.
        //     println()
        //     출력 : 콘솔 출력
        //     출력 : 네트워크 출력
        //     내부의 상세 사항을 잘 몰라도 사용하는데 지장이 없다 --> 추상화가 잘된 것

        // 추상화를 잘 한다는 것은 해당 라이브러리(매서드)를 만든 사람은
        // 구체 사항 또한 잘 알고 있어야 하지만
        // 이것을 사용하는 신입 개발자나 팀원은 상세 사항을 몰라도
        // 사용하는데 큰 문제가 없게 만드는 것을 의미한다.
        // 이것이 잘되면 생산성이 올라가고 유지보수성이 올라간다.
