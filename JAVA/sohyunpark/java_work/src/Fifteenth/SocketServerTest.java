...
public class SocketServerTest {
    public static void main(String[] args) {
        ...
        try {
            ...
            // 네트워크 포트가 16비트를 사용하므로 제한은 0 ~ 65335
            // (단, 고정된 포트를 사용하면 안됨)
            ...
            while(true) {
                // accpet()의 경우 클라이언트가 접속을 요청했는지 체크
                // 만약 요청이 있었다면 요청을 승인한다.
                // (accept()는 블로킹 연산이다)

                // 결국 sock은 클라이언트 소켓을 의미한다.
                // 그래서 좀 더 가독성이 좋은 코드는 Socket clntSock으로 작성하면 좋을듯
                // (전화왔을때 통화하기 슬라이드가 accept()라 보면 됨)
                ...
                // 바뀌지 않으니 항성 고정해서 사용하면 됩니다.
                // 이 버퍼에 클러아인트가 보낸 내용이 들어있다.
            ...
            // 콜 스택(매서드 호출)이 어떤 식으로 이뤄졌는지 상태를 보여줌
            // 디버깅을 위해서 많이 사용하는 편
            e.printStackTrace();
        }
    }
