...
public class ContextSwitching {
    /*
    Context Switching 이란 무엇일까 ?                 이건 요리사가 한명이 여러 요리를 한는것과 요리사 여러명이 하나씩의 요리를 하는게 효율적이라는 것
                                                   요리사는 쓰레드, 한명이 여러 요리를 하기위해 이거하다 저거하다 하면 전에 뭘했는지 까먹는다
                                                   그렇기에 두명이상의 요리사가 자신이 뭘 했는지 기억하고 일을 진행하는 것과같
 ...
 * Mutex vs Spinlock                    8명이서 라면을 요리한다.
                                        한쪽 그룹은 끓여먹는 라면을 먹을거고
                                        한쪽 그룹은 컵라면을 먹을거다
 ...
 결론: 단순 작업 - Spinlock 승음                        스핀락은 문앞에서 대기하고 계속 두드림
      복잡한 작업 - Muex 승
