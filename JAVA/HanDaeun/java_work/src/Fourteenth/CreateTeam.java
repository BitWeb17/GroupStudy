...
public class CreateTeam {
    ...
    public void allocRandomTeam() {
        ...
        //    이제 실제 random 값을 추출하여 ArrayList에 정보를 배치한다.
        // 7) 중복이 되면 안되므로 중복을 판정하는 루틴이 추가적으로 필요하다.

        // 프로젝트: ~~을 만들어서 돈을 법시다.
        // *: ~ 을 어떻게 만들까 ?
        // 앞으로 숙제할 때 요구 사항을 분석하는 연습을 해보면 좋을 것 같습니다.
    ...
    public void allocArrayList(ArrayList<String> al, String[] arr, int loopNum) {
        ...
        for(int i = 0; i < loopNum; i++) {
            // 우리는 A 배열, B 배열이 나눠진 케이스를 받으므로 하나의 배열에서 모든 값을 처리하지 않는다.
