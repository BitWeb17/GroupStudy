...
// 2) 중괄호 내부에는 클래스가 가져야 할 속성을 변수로 만든다.
...
class Person {
    ...
    // 3) 매서드 이름을 작성한다.
    //    마음대로 작명을 해도 되지만 역시 이게 무엇을 하는 것인지는 이름을 보고 알 수 있는 것이 좋다.
    ...
    // 5) 매서드의 기본 골격을 갖췄으므로 실제 매서드가 어떤 작업을 할 것인지 내용을 작성한다.
    public void setName(String name) {
        // 이름이 같을 경우 혼동을 방지하기 위해 this 를 사용한다.
        this.name = name;
    }
    ...
    // void 면 return 이 없다.
    // 그러나 return 이 있다면 넘겨주는 데이터 타입과 return 옆에 오는 클래스 내부 변수의 데이터 타입이 같아야 한다.
