...
import java.util.EmptyStackException;
                        //Runnable이라는 인터페이스를 사용(해당 클래스에는 필수적으로 run함수 필)
class RamenCook implements Runnable{
                //라면의 수
    private int ramenCount;
                //버너의 상태
    private String[] burners = {"stop","stop","stop","stop"};
                    //끓일 라면의 개수를 입력받는다.
    public RamenCook(int count){
        ramenCount = count;
    }
    //Thread에서 진행할 작업을 여기에 정의한
    @Override
    public void run(){
        //끓일 라면의 수가 0이 될때까지
        while(ramenCount > 0) {
            ...
            synchronized (this) {
                ...
                //방지하기 위해 32번줄 synchronized를 만듬 한번에 한 쓰레드만 손댈 수 있
                ramenCount--;
                System.out.println(
                ...
                for (int i = 0; i < burners.length; i++) {
                    if (!burners[i].equals("stop")) continue;
                    //라면을 두개 못가져가게 하는게 아닌
                    //한 쓰레드가 두개의 버너를 차지하지 않게 하기 위해 사용
                    synchronized (this) {
                        //if(burners[i].equals("stop"){
                        //해당 쓰레드의 이름으로 버너를 차지하고 라면을 얹는
