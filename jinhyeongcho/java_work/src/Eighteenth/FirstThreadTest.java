...
// 무조건 Thread가 CPU를 획득하기 위해 경쟁하며 run()이 구동하게 된다.
...
public class FirstThreadTest {
    public static void main(String[] args) {
        // ps -ef 혹은 ps -alx 등을 통해서 프로세를 볼 수 있다.
        // Thread 가 구동되는 것을 보고자 한다면
        // ps -eLf 를 통해 여러 스레드가 존재함을 확인할 수 있다.
        // 윈도우의 경우 프로세스 내부에 여러 스레드가 존재함
        // 반면 유닉스(맥)나 리눅스는 이 부분에 차이가 있는데
        // 프로세스 별개이며 스레드도 별개로 관리한다.
        // 다만 스레드는 프로세스에 소속된 대원 정도로 관리한다.
        // 결국 프로세스는 해당 소속의 대빵을 의미한다.
        ...
        // CPU의 추상화라고 하였다.
        // 왜? CPU가 기계어를 구동시켜야하지만 프로그램 코드가 돌아가니까
        // (운영체제개론을 한 번 공부해보는 것을 권장함)
