...
class Worker implements Runnable {
    ...
    public void run() {
        ...
        try {
            ...
            //      스케쥴링 할 때는 이 Wait Queue에서 스케쥴링을 하게 된다.
...
public class FirstThreadTest {
    public static void main(String[] args) {
        ...
        // 수업 중에 프로세스는 무엇의 추상화라고 했었는지 기억이 나는가 ?
        // CPU의 추상화라고 하였다.
        // 왜 ? CPU가 기계어를 구동시켜야지만 프로그램 코드가 돌아가니까
        // (운영체제개론을 한 번 공부해 보는 것을 권장함)

        // | 음악 듣기 | 게임 하기 | 프로그래밍 하기 |

        // 정상적인 프로그래머라면 반드시 알아야 하는 4가지 VM 영역
        // Stack(스택) : 지역 변수 영역
        // Heap(힙) : 동적 할당(new, malloc)
        // Data(데이터) : static, 전역 변수
        // Text(텍스트) : 기계어, 메서드, 함수

        // 프로세스가 되는 조건이 무엇이었지 ?
        // 우리가 만든 프로그램 코드들이 VM 메모리 영역으로 올라갈 때(load)
        ...
        //     인터페이스를 대충 훑어봐도
        //     이게 뭘 하는구라란 생각이 들어야 하는건가요 ?
        // A : 추상화와 가독성은 다소 차이가 있다.
        //     추상화가 잘 되었다는 것은 이런 의미다.
        //     println()
        //     출력 : 콘솔 출력
        //     출력 : 네트워크 출력
        //     내부의 상세사항을 잘 몰라도 사용하는 데 지장이 없다.

        // 추상화를 잘한다는 것은 해당 라이브러리(메서드)를 만든 사람은
        // 구체사항 또한 잘 알고 있어야 하지만
        // 이것을 사용하는 신입 개발자나 팀원은 상세사항을 몰라도
        // 사용하는데 큰 문제가 없게 만드는 것을 의미한다.
        // 이것이 잘 되면 생산성이 올라가고 유지보수성이 올라간다.

        // 직원이 많아질 수록 생산성이 떨어지는 이유가 뭘까 ?
        // 할 일도 엄청 많고 사람도 많이 필요한다.
        // 오염도가 높아서 그렇다.
