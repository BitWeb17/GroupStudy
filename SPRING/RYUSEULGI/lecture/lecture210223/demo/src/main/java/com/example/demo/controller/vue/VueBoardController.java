...
public class VueBoardController {
    ...
    @PostMapping("")
    public ResponseEntity<VueBoard> register(@Validated @RequestBody VueBoard board, UriComponentsBuilder uriBuilder)
            throws Exception {
         ...
         * .buildAndExpand(board.getBoardNo()) .encode() .toUri(); return
    ...
    @PutMapping("/{boardNo}")
    public ResponseEntity<VueBoard> modify(@PathVariable("boardNo") Long boardNo,
