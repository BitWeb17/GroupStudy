...
public class VueBoardRepository {
    ...
    public List<VueBoard> list() throws Exception {
        ...
        List<VueBoard> results = jdbcTemplate.query("select board_no, title, content, " + "writer, reg_date from board "
                + "where board_no > 0 order by " + "board_no desc, reg_date desc", new RowMapper<VueBoard>() {
                    @Override
